...
static void mountpoint_button_clicked(GtkButton *button, IpodInit *ii) {
    ...
    old_dir = gtk_entry_get_text(GTK_ENTRY (GET_WIDGET (IID_MOUNTPOINT_ENTRY)));

    new_dir = fileselection_get_file_or_dir(_("Select mountpoint"), old_dir, GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);

    if (new_dir) {
        gtk_entry_set_text(GTK_ENTRY (GET_WIDGET (IID_MOUNTPOINT_ENTRY)), new_dir);
        g_free(new_dir);
    }
}

static void set_cell(GtkCellLayout *cell_layout, GtkCellRenderer *cell, GtkTreeModel *tree_model, GtkTreeIter *iter, gpointer data) {
    gboolean header;
    gchar *text;
    IpodInfo *info;
...
gboolean gp_ipod_init(iTunesDB *itdb) {
    ...
    if (mountpoint) {
        gtk_entry_set_text(GTK_ENTRY (GET_WIDGET (IID_MOUNTPOINT_ENTRY)), mountpoint);
    }
    ...
    case GTK_RESPONSE_OK:
        new_mount = g_strdup(gtk_entry_get_text(GTK_ENTRY (GET_WIDGET (IID_MOUNTPOINT_ENTRY))));
        ...
        model = gtk_combo_box_get_active_text(GTK_COMBO_BOX (GET_WIDGET (IID_MODEL_COMBO)));
...
void gp_ipod_init_set_model(iTunesDB *itdb, const gchar *old_model) {
    ...
    switch (response) {
    case GTK_RESPONSE_OK:
        model = gtk_combo_box_get_active_text(GTK_COMBO_BOX (GET_WIDGET (SIMD_MODEL_COMBO)));
        if (!model) {
