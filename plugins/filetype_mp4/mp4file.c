...
typedef MP4FileHandle (*MP4Read_t)(const char* fileName, u_int32_t verbosity DEFAULT(0));

typedef u_int32_t (*MP4GetNumberOfTracks_t)(MP4FileHandle hFile, const char* type DEFAULT(NULL), u_int8_t subType DEFAULT(0));

typedef MP4TrackId (*MP4FindTrackId_t)(MP4FileHandle hFile, u_int16_t index, const char* type DEFAULT(NULL), u_int8_t subType DEFAULT(0));

typedef const char* (*MP4GetTrackType_t)(MP4FileHandle hFile, MP4TrackId trackId);

typedef void (*MP4Close_t)(MP4FileHandle hFile);
...
/* output parameters */
MP4Timestamp* pStartTime DEFAULT(NULL), MP4Duration* pDuration DEFAULT(NULL), MP4Duration* pRenderingOffset DEFAULT(NULL), bool* pIsSyncSample DEFAULT(NULL));

typedef u_int64_t
        (*MP4ConvertFromTrackTimestamp_t)(MP4FileHandle hFile, MP4TrackId trackId, MP4Timestamp timeStamp, u_int32_t timeScale);

typedef u_int64_t
        (*MP4ConvertFromTrackDuration_t)(MP4FileHandle hFile, MP4TrackId trackId, MP4Duration duration, u_int32_t timeScale);

typedef u_int32_t (*MP4GetTrackBitRate_t)(MP4FileHandle hFile, MP4TrackId trackId);

typedef u_int32_t (*MP4GetTrackTimeScale_t)(MP4FileHandle hFile, MP4TrackId trackId);

typedef u_int32_t (*MP4GetTrackMaxSampleSize_t)(MP4FileHandle hFile, MP4TrackId trackId);

typedef MP4SampleId (*MP4GetTrackNumberOfSamples_t)(MP4FileHandle hFile, MP4TrackId trackId);

typedef MP4Timestamp (*MP4GetSampleTime_t)(MP4FileHandle hFile, MP4TrackId trackId, MP4SampleId sampleId);

typedef MP4Duration (*MP4GetTrackDuration_t)(MP4FileHandle hFile, MP4TrackId trackId);
...
typedef bool (*MP4GetMetadataTrack_t)(MP4FileHandle hFile, u_int16_t* track, u_int16_t* totalTracks);
typedef bool (*MP4GetMetadataDisk_t)(MP4FileHandle hFile, u_int16_t* disk, u_int16_t* totalDisks);
...
typedef bool (*MP4GetMetadataCoverArt_t)(MP4FileHandle hFile, u_int8_t **coverArt, u_int32_t* size, uint32_t index DEFAULT(0));
...
typedef bool
        (*MP4GetMetadataFreeForm_t)(MP4FileHandle hFile, const char *name, u_int8_t** pValue, u_int32_t* valueSize, const char *owner DEFAULT(NULL));

typedef bool (*MP4HaveAtom_t)(MP4FileHandle hFile, const char *atomName);

typedef bool (*MP4GetIntegerProperty_t)(MP4FileHandle hFile, const char* propName, u_int64_t *retval);
typedef bool (*MP4GetStringProperty_t)(MP4FileHandle hFile, const char* propName, const char **retvalue);
typedef bool
        (*MP4GetBytesProperty_t)(MP4FileHandle hFile, const char* propName, u_int8_t** ppValue, u_int32_t* pValueSize);
typedef bool (*MP4SetVerbosity_t)(MP4FileHandle hFile, u_int32_t verbosity);
...
typedef bool (*MP4SetMetadataTrack_t)(MP4FileHandle hFile, u_int16_t track, u_int16_t totalTracks);
typedef bool (*MP4SetMetadataDisk_t)(MP4FileHandle hFile, u_int16_t disk, u_int16_t totalDisks);
...
typedef bool (*MP4SetMetadataCoverArt_t)(MP4FileHandle hFile, u_int8_t *coverArt, u_int32_t size);

typedef MP4FileHandle (*MP4Modify_t)(const char* fileName, u_int32_t verbosity DEFAULT(0), u_int32_t flags DEFAULT(0));
...
static gboolean mp4_scan_soundcheck(MP4FileHandle mp4File, Track *track) {
    ...
    if (MP4GetMetadataFreeForm(mp4File, "iTunNORM", &ppValue, &pValueSize, NULL)) {
        gchar *str;
        guint sc1 = 0, sc2 = 0;
    ...
    if (MP4GetMetadataFreeForm(mp4File, "replaygain_track_gain", &ppValue, &pValueSize, NULL)) {
        gchar *str;
        gdouble rg;
        str = g_malloc0((pValueSize + 1) * sizeof(gchar));
        memcpy(str, ppValue, pValueSize * sizeof(gchar));
    ...
    if (mp4File != MP4_INVALID_FILE_HANDLE) {
        ...
        for (track_cur = 0; track_cur < tracks_num; ++track_cur) {
            ...
            if (trackType && ((strcmp(trackType, MP4_AUDIO_TRACK_TYPE) == 0)
                    || (strcmp(trackType, MP4_VIDEO_TRACK_TYPE) == 0) || (strcmp(trackType, MP4_OD_TRACK_TYPE) == 0))) {
...
Track *mp4_get_file_info(const gchar *mp4FileName) {
    ...
    if (mp4File != MP4_INVALID_FILE_HANDLE) {
        ...
        for (track_cur = 0; track_cur < tracks_num; ++track_cur) {
                ...
                for (i = 1; i <= samples; i++) {
                    ...
                    if (!MP4ReadSample(mp4File, trackId, i, &buffer, &m_this_frame_size, NULL, NULL, NULL, NULL)) {
                        ...
                        if (titlelength > 2 && buffer[2] == 0xff && buffer[3] == 0xfe) {
                            titlelength -= 2;
                            gchar *newtitle =
                                    g_utf16_to_utf8((const gunichar2 *) &buffer[4], titlelength, NULL, NULL, NULL);
            ...
            if (trackType && (audio_or_video_found == FALSE) && ((strcmp(trackType, MP4_AUDIO_TRACK_TYPE) == 0)
                    || (strcmp(trackType, MP4_VIDEO_TRACK_TYPE) == 0) || (strcmp(trackType, MP4_OD_TRACK_TYPE) == 0))) {
                gchar *value;
                ...
                double msDuration =
                        (double) MP4ConvertFromTrackDuration(mp4File, trackId, trackDuration, MP4_MSECS_TIME_SCALE);
                ...
                if (prefs_get_int("coverart_apic")) {
                    ...
                    if (MP4GetMetadataCoverArt(mp4File, &image_data, &image_data_len, 0)) {
                        if (image_data) {
...
gboolean mp4_write_file_info(const gchar *mp4FileName, Track *track) {
    ...
    if (mp4File != MP4_INVALID_FILE_HANDLE) {
        ...
        trackType = MP4GetTrackType(mp4File, trackId);
        if (trackType && ((strcmp(trackType, MP4_AUDIO_TRACK_TYPE) == 0) || (strcmp(trackType, MP4_VIDEO_TRACK_TYPE)
                == 0))) {
            gchar *value;
            ...
            gboolean has_tempo = MP4GetMetadataBPM (mp4File,
                    &m_tempo);
#else
            gboolean has_tempo = MP4GetMetadataTempo(mp4File, &m_tempo);
#endif
            gboolean has_compilation = MP4GetMetadataCompilation(mp4File, &m_cpl);
...
#else
            if (has_tempo)
                MP4SetMetadataTempo(mp4File, m_tempo);
#endif
            if (has_compilation)
                MP4SetMetadataCompilation(mp4File, m_cpl);
            if (m_tool)
                MP4SetMetadataTool(mp4File, m_tool);
            if (m_covert)
                MP4SetMetadataCoverArt(mp4File, m_covert, m_size);
