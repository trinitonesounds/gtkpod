...
G_MODULE_EXPORT void open_encoding_dialog (GtkButton *sender, gpointer e)
{
    ...
    init_checkbox (GTK_TOGGLE_BUTTON (gtk_builder_get_object (builder, "use_encoding_for_update")),
                   "update_charset", NULL);

    init_checkbox (GTK_TOGGLE_BUTTON (gtk_builder_get_object (builder, "use_encoding_for_writing")),
                   "write_charset", NULL);
...
G_MODULE_EXPORT void on_customize_tags_clicked (GtkButton *sender, gpointer e)
{
    ...
    if(temp)
    {
        gtk_entry_set_text (GTK_ENTRY (gtk_builder_get_object (builder, "filename_pattern")),
                            temp);

        g_free (temp);
    }

    init_checkbox (GTK_TOGGLE_BUTTON (gtk_builder_get_object (builder, "overwrite_tags")),
...
G_MODULE_EXPORT void on_customize_coverart_clicked (GtkButton *sender, gpointer e)
{
    ...
    if(temp)
    {
        gtk_entry_set_text (GTK_ENTRY (gtk_builder_get_object (builder, "coverart_pattern")),
                            temp);
...
G_MODULE_EXPORT void on_customize_video_thumbnailer_clicked (GtkButton *sender, gpointer e)
{
    ...
    if(temp)
    {
        gtk_entry_set_text (GTK_ENTRY (gtk_builder_get_object (builder, "video_thumbnailer")),
                            temp);
...
G_MODULE_EXPORT void on_conversion_settings_clicked (GtkButton *sender, gpointer e)
{
    ...
    if(temp)
    {
        gtk_file_chooser_set_current_folder (GTK_FILE_CHOOSER (gtk_builder_get_object(builder, "cache_folder")),
                                             temp);

        g_free (temp);
    }

    gtk_spin_button_set_value (GTK_SPIN_BUTTON (gtk_builder_get_object(builder, "bg_threads")),
                               prefs_get_int("file_convert_max_threads_num"));

    gtk_spin_button_set_value (GTK_SPIN_BUTTON (gtk_builder_get_object(builder, "cache_size")),
                               prefs_get_int("file_convert_maxdirsize"));

    for (i = 0; i < COUNTOF(conv_checkbox_map); i++)
    {
        init_checkbox (GTK_TOGGLE_BUTTON (gtk_builder_get_object(builder, conv_checkbox_map[i][0])),
...
static void setup_values ()
{
    ...
    gtk_spin_button_set_value (GTK_SPIN_BUTTON (gtk_builder_get_object (builder, "agp_track_count")),
                               prefs_get_int("misc_track_nr"));

    /* Check boxes */
    for (i = 0; i < COUNTOF(checkbox_map); i++)
    {
        init_checkbox (GTK_TOGGLE_BUTTON (gtk_builder_get_object (builder, checkbox_map[i][0])),
    ...
    gtk_combo_box_set_active (GTK_COMBO_BOX (gtk_builder_get_object (builder, "target_format")),
