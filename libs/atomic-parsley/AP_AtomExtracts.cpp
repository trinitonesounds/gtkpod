...
void APar_ShowMPEG4VisualProfileInfo(TrackInfo* track_info) {
    ...
    if (track_info->parent->movie_info->contains_iods) {
...
/*----------------------
 ...
 All (or whatever was in 14496-2-1988) MPEG-4 Audio (0x40, aka 14496-3) are listed; so is mp3 inside the mp4 container, otherwise generic printout. If an
 'esds' atom was found, then if its section 5 length is known, then 'esds' was carrying channel info (in which case, channels was listed twice, so prefer this
 listing); otherwise fall back to 'mp4a' carrying channel info (indicated by the parens() around it).
 ----------------------*/
void APar_ShowMPEG4AACProfileInfo(TrackInfo* track_info) {
    if (track_info->descriptor_object_typeID == 1) {
        ...
        fprintf(stdout, "  MPEG-4 AAC Low Complexity Profile"); //most files will land here
    }
    else if (track_info->descriptor_object_typeID == 3) {
        fprintf(stdout, "  MPEG-4 AAC Scaleable Sample Rate Profile");
    }
    else if (track_info->descriptor_object_typeID == 4) {
        fprintf(stdout, "  MPEG-4 AAC T/F Profile");
    }
    else if (track_info->descriptor_object_typeID == 5) {
        fprintf(stdout, "  MPEG-4 AAC T/F Main Profile");
    }
    else if (track_info->descriptor_object_typeID == 6) {
        fprintf(stdout, "  MPEG-4 AAC T/F LC Profile");
    }
    else if (track_info->descriptor_object_typeID == 7) {
        fprintf(stdout, "  MPEG-4 AAC TwinVQ Profile");
    }
    else if (track_info->descriptor_object_typeID == 8) {
        fprintf(stdout, "  MPEG-4 AAC CELP Profile");
    }
    else if (track_info->descriptor_object_typeID == 9) {
        fprintf(stdout, "  MPEG-4 AAC HVXC Profile");
    }
    else if (track_info->descriptor_object_typeID == 10) {
        fprintf(stdout, "  MPEG-4 AAC HILN Profile");
    }
    else if (track_info->descriptor_object_typeID == 11) {
...
void APar_ShowObjectProfileInfo(uint8_t track_type, TrackInfo* track_info) {
    if (track_info->contains_esds) {
        ...
        default: {
            ...
            if (track_info->parent->movie_info->contains_iods && iods_info.audio_profile == 0xFE) {
...
/*----------------------
 ...
 ----------------------*/
void calculate_sample_size(char* uint32_buffer, FILE* isofile, short stsz_atom, TrackInfo *track_info) {
    ...
    if (sample_size == 0) {
        for (uint32_t atom_offset = 20; atom_offset < parsedAtoms[stsz_atom].AtomicLength; atom_offset += 4) {
            total_size += (uint64_t) APar_read32(uint32_buffer, isofile, parsedAtoms[stsz_atom].AtomicStart
                    + atom_offset);
        }
    }
    else {
    ...
    track_info->sample_aggregate = total_size;
}
...
void APar_Extract_AMR_Info(char* uint32_buffer, FILE* isofile, short track_level_atom, TrackInfo* track_info) {
    ...
    if (track_info->track_codec == 0x73616D72 || track_info->track_codec == 0x73617762) { //samr or sawb contain modes only
        track_info->amr_modes = APar_read16(uint32_buffer, isofile, parsedAtoms[track_level_atom].AtomicStart
                + amr_specific_offet + 4 + 1);
    }
    return;
}
...
void APar_Extract_esds_Info(char* uint32_buffer, FILE* isofile, short track_level_atom, TrackInfo* track_info) {
    ...
    while (offset_into_stsd < parsedAtoms[track_level_atom].AtomicLength) {
        offset_into_stsd++;
        if (APar_read32(uint32_buffer, isofile, parsedAtoms[track_level_atom].AtomicStart + offset_into_stsd)
                == 0x65736473) {
            ...
            if (section4_length <= section3_length && section4_length != 0) {
                ...
                if (section4_length == 0x9D)
                    offset_into_esds++; //upper limit? when gpac puts an ogg in, section 3 is 9D - so is sec4 (section 4 real length with ogg = 0x0E86)
            ...
            if (APar_read8(isofile, esds_start + offset_into_esds) == 0x05) {
                ...
                if (section5_length <= section4_length && section5_length != 0) {
                    ...
                    if (track_info->type_of_track & AUDIO_TRACK) {
                        ...
                        track_info->channels = (uint16_t) APar_ExtractChannelInfo(isofile, esds_start
                                + offset_into_esds);
...
void APar_ExtractTrackDetails(char* uint32_buffer, FILE* isofile, Trackage* track, TrackInfo* track_info) {
    ...
    if (APar_read8(isofile, parsedAtoms[track->track_atom].AtomicStart + 8) == 0) {
        ...
        track_info->creation_time = APar_read32(uint32_buffer, isofile, parsedAtoms[track->track_atom].AtomicStart
                + 12);
    ...
    track_info->media_sample_rate = APar_read32(uint32_buffer, isofile, parsedAtoms[track->track_atom].AtomicStart + 20);
    ...
    if (parsedAtoms[track->track_atom].AtomicLength > 34) {
        ...
        APar_readX(track_info->track_hdlr_name, isofile, parsedAtoms[track->track_atom].AtomicStart + 32, parsedAtoms[track->track_atom].AtomicLength
                - 32);
    }
    ...
    if (track_info->type_of_track & VIDEO_TRACK) { //vide
        track_info->video_width = APar_read16(uint32_buffer, isofile, parsedAtoms[track->track_atom + 1].AtomicStart
                + 32);
        track_info->video_height = APar_read16(uint32_buffer, isofile, parsedAtoms[track->track_atom + 1].AtomicStart
                + 34);
        ...
        if (track_info->track_codec == 0x61766331 || track_info->track_codec == 0x64726D69) { //avc1 or drmi
            ...
            if (memcmp(parsedAtoms[track->track_atom].AtomicName, "esds", 4) == 0) {
                ...
                track_info->type_of_track = VIDEO_TRACK;
                    ...
                    || track_info->track_codec == 0x73617770) { //samr, sawb or sawp
                track_info->type_of_track = AUDIO_TRACK;
            }
            else {
        ...
        if (track_info->track_codec == 0x73616D72 || track_info->track_codec == 0x73617762
                || track_info->track_codec == 0x73617770) { //samr or sawb (sawp doesn't contain modes)
    ...
    if (((track_info->type_of_track & AUDIO_TRACK) || (track_info->type_of_track & VIDEO_TRACK))
            && track_info->avg_bitrate == 0) {
        if (track_info->track_codec == 0x616C6163) { //alac
            track_info->channels = APar_read16(uint32_buffer, isofile, parsedAtoms[track->track_atom + 1].AtomicStart
                    + 24);
        }
    }
    ...
    if (memcmp(parsedAtoms[track->track_atom].AtomicName, "stsz", 4) == 0) {
        calculate_sample_size(uint32_buffer, isofile, track->track_atom, track_info);
    }
    ...
    if (track_info->track_codec >= 0x64726D00 && track_info->track_codec <= 0x64726DFF) {
        ...
        memset(uint32_buffer, 0, 5);
        track_info->protected_codec = APar_read32(uint32_buffer, isofile, parsedAtoms[track->track_atom].AtomicStart
                + 8);
    }
    ...
    if (track_info->contains_esds) {
        ...
        if (_offset > 0 && _offset < parsedAtoms[track->track_atom].AtomicLength) {
            ...
            APar_readX(track_info->encoder_name, isofile, parsedAtoms[track->track_atom].AtomicStart + _offset, parsedAtoms[track->track_atom].AtomicLength
                    - _offset);
        }
    }
    return;
...
/*----------------------
 ...
 ----------------------*/
void APar_ExtractMovieDetails(char* uint32_buffer, FILE* isofile, MovieInfo *movie_info, AtomicInfo* mvhd_atom) {
...
#if defined (_MSC_VER)
    return (( (double)((__int64)track_info->sample_aggregate) /
                    ( (double)((__int64)track_info->duration) / (double)((__int64)movie_info.timescale)) ) / 1000.0 * 8);
#else
        return (((double) track_info->sample_aggregate
                / ((double) track_info->duration / (double) track_info->parent->movie_info->timescale)) / 1000.0 * 8);
#endif
    }
}
...
void APar_Print_TrackDetails(TrackInfo* track_info) {
    double bitrate = APar_calculate_bitrate(track_info);
    fprintf(stdout, "     %.2f kbp/s", bitrate);

    fprintf(stdout, "  %.3f sec", (float) track_info->duration / (float) track_info->parent->movie_info->timescale);
    ...
    if (track_info->track_codec == 0x6D703476) { //mp4v profile
        ...
        APar_ShowObjectProfileInfo(S263_TRACK, track_info);
            ...
            || track_info->track_codec == 0x73617770) { //samr,sawb or sawp in 3gp
    ...
    if ((track_info->type_of_track & VIDEO_TRACK)
            && ((track_info->max_bitrate > 0 && track_info->ObjectTypeIndication == 0x20)
...
Trackage* APar_ExtractDetails(FILE* isofile, uint8_t optional_output) {
    char* uint32_buffer = (char*) malloc(sizeof(char) * 5);
    Trackage *track = (Trackage *) malloc(sizeof(Trackage));
    MovieInfo movie_info = {0};

    track->total_tracks = 0;
    track->track_num = 0;
    track->track_atom = 0;
    track->movie_info = &movie_info;
    track->infos = NULL;

    AtomicInfo* mvhdAtom = APar_FindAtom("moov.mvhd", false, VERSIONED_ATOM, 0);
    if (mvhdAtom != NULL) {
        APar_ExtractMovieDetails(uint32_buffer, isofile, track->movie_info, mvhdAtom);
        fprintf(stdout, "Movie duration: %.3lf seconds (%s) - %.2lf* kbp/sec bitrate (*=approximate)\n", track->movie_info->seconds, secsTOtime(track->movie_info->seconds), track->movie_info->simple_bitrate_calc);
    }

    AtomicInfo* iodsAtom = APar_FindAtom("moov.iods", false, VERSIONED_ATOM, 0);
    if (iodsAtom != NULL && track->movie_info != NULL) {
        track->movie_info->contains_iods = true;
    ...
    if (optional_output & SHOW_TRACK_INFO) {
        ...
        track->infos = (TrackInfo **) calloc(track->total_tracks, sizeof(TrackInfo));

        while (track->total_tracks > track->track_num) {

            TrackInfo *track_info = (TrackInfo *) malloc(sizeof(TrackInfo));
            track->infos[track->track_num] = track_info;
            track_info->parent = track;
